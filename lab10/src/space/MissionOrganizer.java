...
public class MissionOrganizer implements Iterable<Mission> {
	...
	public void add(Mission mission) {
		if (!this.contains(mission)) {
			if (mission.getCost() < ONE_TRILLION) {
				...
				costlyMissions.add(mission);
			}
		}

	}

	// Create an ArrayList<Mission>. Add all the cheap missions to the ArrayList;
	// then
	// add all the expensive missions. (Hint: look in the API page for ArrayList for
	// a
	// method that adds all members of a collection.) The ArrayList will then
	// contain
	...
	public Iterator<Mission> iterator() {
		ArrayList<Mission> sorted = new ArrayList<Mission>();
		sorted.addAll(cheapMissions);
		sorted.addAll(costlyMissions);
	...
	private final static Mission[] TEST_MISSIONS = { 
			...
			new Mission("Tabby's Star", 7.11E20f),
			new Mission("EPIC 204278916", 5.7E17f),
			new Mission("Neverland", .99E9F),
			new Mission("Planet X", 1.7E11f)};
